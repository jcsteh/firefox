  ...
  if (aCacheDomain & CacheDomain::Viewport && IsDoc()) {
    ...
    if (nsIFrame* rootFrame = presShell->GetRootFrame()) {
      ...
      for (nsIFrame* frame : frames) {
        if (!frame->IsTextFrame() && !frame->IsPrimaryFrame()) {
          ...
          // frames. We don't want this Accessible to appear before leaves on
          // other lines which are later in the `frames` array. Otherwise, when
          // hit testing, this Accessible will match instead of those leaves. We
          // will add this Accessible when we get to its primary frame later.
          continue;
        }
