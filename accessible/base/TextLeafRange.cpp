...
FindDOMTextOffsetAttributes(LocalAccessible* aAcc, int32_t aRenderedStart,
  ...
  size_t highlightCount = frameSel->GetHighlightSelectionCount();
  ...
  for (size_t h = 0; h < highlightCount; ++h) {
    RefPtr<dom::Selection> domSel = frameSel->GetHighlightSelection(h);
    MOZ_ASSERT(domSel);
    nsStaticAtom* attr = nullptr;
    MOZ_ASSERT(domSel->GetHighlightSelectionData().mHighlight);
    switch (domSel->GetHighlightSelectionData().mHighlight->Type()) {
...
void TextLeafPoint::AddTextOffsetAttributes(AccAttributes* aAttrs) const {
  ...
  // Ranges can overlap each other. Therefore, even though offsetAttrs is sorted
  // by mStartOffset, we can't binary search because that won't consider
  // mEndOffset, which can't be sorted as well.
  for (const TextOffsetAttribute& range : *offsetAttrs) {
    ...
    if (range.mEndOffset != -1 && range.mEndOffset <= mOffset) {
      continue;
    }
...
TextLeafPoint TextLeafPoint::FindTextOffsetAttributeSameAcc(
  ...
  for (const TextOffsetAttribute& range : *offsetAttrs) {
    ...
    if (aDirection == eDirNext) {
      // Find the closest range offset > mOffset.
      if (range.mStartOffset > mOffset) {
      ...
      if (range.mStartOffset != -1 && range.mStartOffset > dest) {
        dest = range.mStartOffset;
      }
    }
  }
