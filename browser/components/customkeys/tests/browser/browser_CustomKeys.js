...
add_task(async function testChangeKey() {
  ...
  info(`Changing key_gotoHistory to ${unusedDisplay}`);
  CustomKeys.changeKey("key_gotoHistory", unusedModifiers, unusedKey);
  Assert.deepEqual(
    CustomKeys.getDefaultKey("key_gotoHistory"),
    [historyModifiers, "H"],
  ...
  EventUtils.synthesizeKey(unusedKey, unusedOptions, window);
  await focused;
  ...
  info(`Changing viewBookmarksSidebarKb to ${historyDisplay}`);
  CustomKeys.changeKey("viewBookmarksSidebarKb", historyModifiers, "H");
  Assert.deepEqual(
    CustomKeys.getDefaultKey("viewBookmarksSidebarKb"),
    ["accel", "B"],
    "viewBookmarksSidebarKb is customized"
  );
  info(`Pressing ${historyDisplay}`);
  focused = BrowserTestUtils.waitForEvent(window, "SidebarFocused");
  EventUtils.synthesizeKey("H", historyOptions, window);
  await focused;
  ok(true, "Sidebar got focus");
  is(
    SidebarController.currentID,
...
add_task(async function testClearKey() {
  ...
  CustomKeys.changeKey("focusURLBar", "", "");
  Assert.deepEqual(
    CustomKeys.getDefaultKey("focusURLBar"),
    ["accel", "L"],
...
add_task(async function testOtherWindow() {
  ...
  info(`Pressing ${unusedDisplay} in new window`);
  let focused = BrowserTestUtils.waitForEvent(newWin, "SidebarFocused");
  EventUtils.synthesizeKey(unusedKey, unusedOptions, newWin);
  await focused;
  ...
  info(`Pressing ${historyDisplay} in new window`);
  focused = BrowserTestUtils.waitForEvent(newWin, "SidebarFocused");
  EventUtils.synthesizeKey("H", historyOptions, newWin);
  await focused;
  ok(true, "Sidebar got focus in new window");
  is(
    newWin.SidebarController.currentID,
