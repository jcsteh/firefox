...
addAboutKbTask(async function testChange(tab) {
  ...
  await SpecialPowers.spawn(tab, [consts], async _consts => {
    ...
    info(`Pressing ${_consts.unusedModifiersDisplay}`);
    let selected = ContentTaskUtils.waitForEvent(input, "select");
    EventUtils.synthesizeKey(..._consts.unusedModifiersArgs, content);
    await selected;
    is(
      input.value,
    ...
    focused = ContentTaskUtils.waitForEvent(change, "focus");
    EventUtils.synthesizeKey(_consts.unusedKey, _consts.unusedOptions, content);
    await focused;
...
addAboutKbTask(async function testConflictingChange(tab) {
  ...
  await SpecialPowers.spawn(tab, [consts], async _consts => {
    ...
    EventUtils.synthesizeKey("H", _consts.historyOptions, content);
  });
  await handled;
  await SpecialPowers.spawn(tab, [consts], async _consts => {
